...
namespace NinjaTrader.NinjaScript.AddOns.Ninjex.PremarketRange.Analysis
{
    ...
    {
        private readonly Dictionary<TradeDirection, int> attemptCounts =
            new Dictionary<TradeDirection, int>();
        ...
        public IList<BreakoutEvent> Detect(
            ...
            int minimumDistanceTicks)
        {
            ...
            var longQualifies =
                context.Bar.Close >= highThreshold;

            var shortQualifies =
                context.Bar.Close <= lowThreshold;

            // Rearm only after a completed candle returns to the
            // original range level or inside the range.
        ...
        private BreakoutEvent CreateEvent(
            ModelBarContext context,
            TradeDirection direction)
        {
            var attempt = 1;

            if (attemptCounts.TryGetValue(
                    direction,
                    out var count))
            {
                attempt = count + 1;
            ...
            var distanceTicks =
                direction == TradeDirection.Long
                    ? (context.Bar.Close - level)
                      / context.Session.TickSize
                    : (level - context.Bar.Close)
                      / context.Session.TickSize;

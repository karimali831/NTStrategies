...
namespace NinjaTrader.NinjaScript.AddOns.Ninjex.Menu
{
    ...
    public sealed class MenuManager : IDisposable
    {
        ...
        private const string SuiteSeparatorAutomationId = "Ninjex_ToolsSeparator";
            ...
            for (var i = toolsRoot.Items.Count - 1; i >= 0; i--)
            {
                ...
                if (obj is MenuItem mi)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(id) &&
                        id.StartsWith("Ninjex_", StringComparison.Ordinal))
                    {
        ...
        private static MenuItem FindFirstNativeMenuItem(MenuItem toolsRoot)
        {
            ...
            foreach (var obj in toolsRoot.Items)
            {
                ...
                var id = AutomationProperties.GetAutomationId(mi);
                if (!string.IsNullOrWhiteSpace(id) && id.StartsWith("Ninjex_", StringComparison.Ordinal))
                    continue;

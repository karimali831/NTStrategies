...
namespace NinjaTrader.NinjaScript.AddOns.Ninjex.Tools.RelayTool
{
    public partial class RelayTool
    {
        ...
        private static string CrashLogPath =>
            Path.Combine(Globals.UserDataDir, "log", "RelayTool.crash.log");
        ...
        private static void LogUnhandled(string scope, Exception ex)
        {
            try
            {
                if (ex == null)
                {
                    NinjexRuntime.PrintLog($"[UNHANDLED] scope={scope} ex=<null>");
                    return;
                }

                NinjexRuntime.PrintLog($"[UNHANDLED] scope={scope}");
                ...
                while (inner != null)
                {
                    ...
                    NinjexRuntime.PrintLog(inner.ToString());

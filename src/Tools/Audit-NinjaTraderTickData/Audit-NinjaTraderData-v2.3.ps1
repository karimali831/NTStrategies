...
<#
Audit-NinjaTraderData-v2.3.ps1 -- READ ONLY. No downloads/deletes/database writes.

FOCUSED CHECK (candidate filename mapping; independent exports recommended):
 .\Audit-NinjaTraderData-v2.3.ps1 -Symbols ES -From '2026-08-27' -To '2026-09-01'
...
requested customary Monday-roll allocation. Same-contract warm-up is required.
Roll convention: https://www.cmegroup.com/trading/equity-index/rolldates.html
Allocation uses ET session dates, including the previous evening. Files on
other expiries are real separate instruments, never move or rename them to repair
a gap. Download the assigned expiry instead. Overlaps describe container coverage,
not duplicate executions or proof that prices are identical.
New reports: contract-windows.csv, contract-date-audit.csv, contract-overlaps.csv,
contract-allocation-review.csv. Allocation checks cover requested ET session dates.
Daily minute containers may straddle two sessions; each is classified separately.
Optional -HashFiles -CompareInventory 'C:\AuditPC\inventory.csv'.
v2.3 changes: customary Monday roll windows; per-session contract allocation
and overlapping-contract reports for Tick+Last and Minute+Last.
Retains sourced closed-period exclusions and provider availability reporting;
...
Write-Host 'NinjaTrader Data Audit v2.3 - hourly tick checks + minute content gaps + overnight dependencies' -ForegroundColor Cyan
...
foreach($c in $contracts) {
 foreach($interval in @('tick','minute')) {
  ...
  if(-not (Test-Path -LiteralPath $folder -PathType Container)) {if($c.From -le $To -and $c.To -ge $From){Issue 'MissingFolder' $folder};continue}
...
# Allocate by actual ET session, never by the raw filename calendar date alone.
$contractReport=@($contracts | ForEach-Object {[pscustomobject]@{Contract=$_.Contract;From=$_.From.ToString('yyyy-MM-dd');To=$_.To.ToString('yyyy-MM-dd');FirstEveningET=$_.From.AddDays(-1).AddHours(18).ToString('yyyy-MM-dd HH:mm')}})
SaveCsv $contractReport 'contract-windows.csv' @('Contract','From','To','FirstEveningET')
$ownerByDate=@{}
foreach($c in $contracts){
 for($d=$c.From;$d -le $c.To;$d=$d.AddDays(1)){
  if($d.DayOfWeek -in @('Saturday','Sunday')){continue}
  $k="$($c.Symbol)|$($d.ToString('yyyy-MM-dd'))"
  if($ownerByDate.ContainsKey($k)){throw "Duplicate assigned session: $k"}
  $ownerByDate[$k]=$c.Contract
 }
}
$warmupByDate=@{}
foreach($w in $windows){if($w.Basis -eq 'WarmupDependency'){$warmupByDate["$($w.Contract)|$($w.DateET)"]=$true}}
$dateAudit=[Collections.Generic.List[object]]::new()
foreach($r in $inventory){
 if($r.PriceType -ne 'Last' -or -not $r.FileDate){continue}
 $format=if($r.Interval -eq 'tick' -and $r.RawStamp.Length -eq 12){'yyyyMMddHHmm'}elseif($r.Interval -eq 'minute' -and $r.RawStamp.Length -eq 8){'yyyyMMdd'}else{Issue 'AllocationMappingUnsupported' $r.RelativePath;continue}
 $label=[datetime]::ParseExact($r.RawStamp,$format,$ci)
 if($r.Interval -eq 'tick'){
  if($label.Minute -ne 0){Issue 'AllocationMappingUnsupported' $r.RelativePath;continue}
  $lo=if($TickFileHourLabel -eq 'End'){$label.AddHours(-1)}else{$label}
  $hi=$lo.AddHours(1)
 }else{
  $lo=if($MinuteFileDateLabel -eq 'CloseDate'){$label.AddMinutes(-1)}else{$label}
  $hi=$lo.AddDays(1)
 }
 $covered=@{}
 for($st=$lo;$st -lt $hi;$st=$st.AddMinutes(1)){
  $unspecified=[datetime]::SpecifyKind($st,[DateTimeKind]::Unspecified)
  if($storageZone.IsInvalidTime($unspecified) -or $storageZone.IsAmbiguousTime($unspecified)){continue}
  $utc=[TimeZoneInfo]::ConvertTimeToUtc($unspecified,$storageZone)
  $t=[TimeZoneInfo]::ConvertTimeFromUtc($utc,$et)
  if($t.Hour -eq 17 -or (IsClosed $t)){continue}
  $session=if($t.Hour -ge 18){$t.Date.AddDays(1)}else{$t.Date}
  if($session.DayOfWeek -in @('Saturday','Sunday') -or $session -lt $From -or $session -gt $To){continue}
  $date=$session.ToString('yyyy-MM-dd')
  if(-not $covered.ContainsKey($date)){$covered[$date]=@{First=$t;Through=$t.AddMinutes(1)}}
  else{$covered[$date].Through=$t.AddMinutes(1)}
 }
 foreach($date in $covered.Keys){
  $symbol=$r.Contract.Split(' ')[0];$ownerKey="$symbol|$date"
  $owner=if($ownerByDate.ContainsKey($ownerKey)){$ownerByDate[$ownerKey]}else{''}
  $status=if(-not $owner){'UNASSIGNED_SESSION'}elseif($owner -eq $r.Contract){'ASSIGNED_CONTRACT'}elseif($warmupByDate.ContainsKey("$($r.Contract)|$date")){'WARMUP_ON_NONLEAD_CONTRACT'}else{'OUTSIDE_SELECTED_CONTRACT_WINDOW'}
  $span=$covered[$date]
  $dateAudit.Add([pscustomobject]@{Symbol=$symbol;SessionET=$date;Interval=$r.Interval;DataType='Last';StoredContract=$r.Contract;AssignedContract=$owner;Status=$status;ContainerFromET=$span.First.ToString('yyyy-MM-dd HH:mm');ContainerThroughET=$span.Through.ToString('yyyy-MM-dd HH:mm');RelativePath=$r.RelativePath;FileProblem=$r.Problem;Mapping=$mappingStatus})
 }
}
SaveCsv $dateAudit 'contract-date-audit.csv' @('Symbol','SessionET','Interval','DataType','StoredContract','AssignedContract','Status','ContainerFromET','ContainerThroughET','RelativePath','FileProblem','Mapping')
$allocationReview=@($dateAudit | Where-Object Status -ne 'ASSIGNED_CONTRACT' | Sort-Object -Property @('SessionET','Interval','StoredContract'))
SaveCsv $allocationReview 'contract-allocation-review.csv' @('SessionET','Interval','StoredContract','AssignedContract','Status','ContainerFromET','ContainerThroughET','RelativePath','FileProblem','Mapping')
$overlaps=@(foreach($g in ($dateAudit | Group-Object -Property @('Symbol','SessionET','Interval'))){
 $stored=@($g.Group.StoredContract | Sort-Object -Unique)
 if($stored.Count -lt 2){continue}
 $first=$g.Group[0]
 [pscustomobject]@{Symbol=$first.Symbol;SessionET=$first.SessionET;Interval=$first.Interval;DataType='Last';StoredContracts=($stored -join '; ');AssignedContract=$first.AssignedContract;Status='MULTIPLE_CONTRACT_CONTAINERS_FOR_SESSION';Meaning='Container session coverage only; separate expiries are not interchangeable';Mapping=$mappingStatus}
})
SaveCsv $overlaps 'contract-overlaps.csv' @('Symbol','SessionET','Interval','DataType','StoredContracts','AssignedContract','Status','Meaning','Mapping')
Write-Host 'CONTRACT ALLOCATION - session dates; data is never moved or deleted' -ForegroundColor Cyan
$contractReport | Format-Table -AutoSize | Out-String -Width 200 | Write-Host
Write-Host "Overlapping session/interval rows: $($overlaps.Count). See contract-overlaps.csv."
Write-Host "Non-assigned file/session rows: $($allocationReview.Count). See contract-allocation-review.csv (warm-up explicitly distinguished)."
if($overlaps.Count){$overlaps | Select-Object -Property @('SessionET','Interval','StoredContracts','AssignedContract') | Format-Table -AutoSize | Out-String -Width 200 | Write-Host}
# Existing completeness checks below still require files from the assigned expiry.
# Presence on another expiry does not satisfy those checks.
...
$display=@(
 'MISSING DATA / REVIEW CHECKLIST - v2.3'
...
@"
NinjaTrader data audit v2.3 - $(Get-Date -Format o)
Database: $DbRoot
...
Use ContractsCsv to override the customary roll allocations.
contract-date-audit.csv maps containers to assigned ET sessions within requested scope.
contract-overlaps.csv lists session/interval coverage on multiple expiries.
contract-allocation-review.csv distinguishes nonlead warm-up from out-of-window data.
These are allocation findings, not proof of incorrect instrument contents.
Do not move or rename another expiry's data to fill an assigned-contract gap.
